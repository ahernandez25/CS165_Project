...
public:
  void set_num_hashf (int k1)	//will set num hash functions
  {
    ...
    Bloom_fil=temp; //fill bloom to 0
  	...
  	uint32_t Hash(string concatenate_O_P, uint32_t seed) // hash strings using mrmurhash
	{
        //uint32_t seed = seed;
        uint32_t hash[1];
  ...
  vector < uint32_t > hash_k (string obj_name)	// creates k num of has functions
  {
    vector < uint32_t > temp;	//hold to return 
    uint32_t seed_hold = seed;	//gives different hashs depending on seed val
    //int32_t hash[1];		//stores k num of hash
